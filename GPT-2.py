...
class GPT(nn.Module):
  def __init__(self,config):
    ...
    self.transformer= nn.ModuleDict(dict(
        ...
        wpe=nn.Embedding(config.block_size,config.n_embd), # postition embedding
  ...
  def _init_weights(self,module):
    if isinstance(module,nn.Linear) :
      std=0.02
      if hasattr(module, 'NANOGPT_SCALE_INIT'): # for linear layers this is how to init weights
        std *= (2 * self.config.n_layer) ** -0.5
      torch.nn.init.normal_(module.weight, mean=0.0, std=std)
      if module.bias is not None:
        torch.nn.init.zeros_(module.bias)
    elif isinstance(module, nn.Embedding):
      torch.nn.init.normal_(module.weight, mean=0.0, std=0.02) # for non - linear layers this is how to init weights

  def Forward(self,idx,targets=None):
    #idx-B,T
    B,T=idx.size
    assert T<=self.config.block_size, "Cannot forward, model block size is exhausted."
    pos=torch.arrange(0,T,dtype=torch.long,device=idx.device) # did this to get a token list 
